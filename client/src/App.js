...
function App() {
    ...
    if (check) {
      toast(
        t => (
          <p>
            <IconButton
              sx={{ marginLeft: "17.5rem", marginTop: "-25px" }}
              onClick={() => toast.dismiss(t.id)}
            >
              <RiCloseLine />
            </IconButton>
            <br />A comic you have on your wishlist has been posted for
            sale/trade!
          </p>
        ),
        {
  ...
  React.useEffect(() => {
    if (!listingsLoading && listingsData && listingsData.posts) {
      dispatch({
        type: UPDATE_POSTS,
      ...
      console.log("Data not ready:", { listingsLoading, listingsData });
    }
          ...
          <div>
            ...
            <Routes>
              ...
              <Route
                path="/login"
                element={
                  <>{Auth.loggedIn() ? <Navigate to="/" /> : <SignInSide />}</>
                }
              />
              <Route
                path="/signup"
              ...
              <Route
                ...
                element={
                  <>
                    {Auth.loggedIn() ? <Profile /> : <Navigate to="/login" />}
                  </>
                }
              />
              <Route
              ...
              <Route
                ...
                element={
                  <>
                    {Auth.loggedIn() ? <Listings /> : <Navigate to="/login" />}
                  </>
                }
              />
              <Route
                path="/"
                element={
                  <>{Auth.loggedIn() ? <Main /> : <Navigate to="/login" />}</>
                }
              />
